<#
.SYNOPSIS
Release PsychImGui 0.2.0: set the version, verify, push, tag, and check the published zip.
...
Off by default: the script only starts from a tree identical to a green
origin/main, and CI runs the full matrix again on the version commit before
the tag, so the local run repeats what CI has done and will do.
...
$Name        = 'PsychImGui'                          # tag and release title prefix
$MexName     = 'PsychImGui'                          # the MEX function
$VersionField = 'psychimgui'                         # field of ('Version') that carries the version
$SetupName   = 'PsychImGuiSetup'                     # setup function at the zip root
$WindowsZip  = 'psychimgui-matlab-windows.zip'       # asset checked after the release
$VersionEdits = @(
    @{ File = 'src/core/psychimgui_core.cpp'; Pattern = '#define PSYCHIMGUI_VERSION_STR "\d+\.\d+\.\d+"'; Replacement = '#define PSYCHIMGUI_VERSION_STR "{V}"' }
)
$NeedsGen    = $false                                # the generator stamps the version into sources
$LocalTestRuns = @(
...
# ---- 2. verify locally ------------------------------------------------------
if (-not $LocalTests) {
    Step "Local tests not requested; CI on the version commit is the check"
} elseif ($DryRun) {
