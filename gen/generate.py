...
def write_help(root, rows, helps, imgui_ver, implot_ver):
    ...
    for name, sig in helps:
        L.append(f"%     {sig}")
    L.append("%")
    L.append("%   See also PsychImGuiInput, PsychImGuiFrame, PsychImGuiKeymap,")
    L.append("%   PsychImGuiOp, PsychImGuiDemo.")
    L.append("")
    L.append("    error('psychimgui:NotBuilt', ...")
    L.append("        ['The PsychImGui MEX is not on the path. Run build, then add ' ...")
    L.append("         'the dist directory to the path.']);")
    L.append("end")
